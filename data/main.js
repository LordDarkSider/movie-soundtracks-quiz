...
function next(){
    ...
    if(done.length == nb_movies){return end()};
    num = select_alea();
...
function end(){
    ...
    if(mode=='mcq'){var score_brut = (String(score)+'/2000'); var prctg = (String(Math.round(score/20))+'%')}
    else if(mode=='normal'){var score_brut = (String(score)+'/6000'); var prctg = (String(Math.round(score/60))+'%')}
    else if(mode=='trial'){return score_end_label.innerHTML = ('Score: '+String(score));}
    score_end_label.innerHTML = ('Score: '+score_brut+' ('+prctg+')');
};
...
function home(){
    if(done.length > 0){var language = selectlang.value; var msg = config_language.get('confirm').get(language); conf= confirm(msg)}
    else{var conf = 1;};
